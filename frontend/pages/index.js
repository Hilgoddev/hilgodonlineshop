...
export default function Home({ products, categories = [], campaigns = [], categorySections = {} }) {
  ...
  return (
    ...
    >
      ...
      {/* 4-Mini Banners. Original themes preserved — only the LINKS were fixed:
          phones/laptops/gadgets aren't real categories (dead pages), so those
          three point to electronics, where tech inventory lives and grows.
          Fashion uses the combined menswear+womenswear alias. */}
      <div className="banner-4col">
        ...
        <div className="mini-banner" onClick={() => window.location.href = '/products?category=electronics'} style={{ background: 'linear-gradient(135deg,#1d4ed8,#1e3a8a)' }}>
        ...
        <div className="mini-banner" onClick={() => window.location.href = '/products?category=electronics'} style={{ background: 'linear-gradient(135deg,#047857,#065f46)' }}>
