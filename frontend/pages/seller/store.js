...
export default function SellerStore() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setStore(data.data);
        setModal({ open: true, title: 'Saved', message: 'Store profile saved successfully!', type: 'alert', onConfirm: closeModal });
      }
    } catch (err) {
      console.error(err);
      setModal({ open: true, title: 'Error', message: 'Failed to save store profile.', type: 'alert', danger: true, onConfirm: closeModal });
    }
  };
