        ...
        <div className="seller-metrics-grid" style={{ marginBottom: '20px' }}>
          ...
          <div className="card" style={{ padding: '20px' }}>
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>
              ₦{orders.reduce((s, o) => s + o.sellerTotal, 0).toLocaleString()}
            </div>
          </div>
          <div className="card" style={{ padding: '20px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'var(--gray-6)', flexWrap: 'wrap', gap: '8px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontWeight: 800, color: 'var(--primary)', fontSize: '.95rem' }}>₦{order.sellerTotal.toLocaleString()}</div>
                      ...
                      {order.items.map((item, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 0', borderBottom: i < order.items.length - 1 ? '1px solid var(--gray-4)' : 'none' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: '.78rem', color: 'var(--gray-1)' }}>Qty: {item.quantity} · ₦{Number(item.price).toLocaleString()} each</div>
                          </div>
                          <div style={{ fontWeight: 700, fontSize: '.9rem', flexShrink: 0 }}>₦{(item.price * item.quantity).toLocaleString()}</div>
                        </div>
                      ))}
                    </div>
                  </div>
