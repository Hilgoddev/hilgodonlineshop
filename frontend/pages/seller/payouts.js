        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' }}>
              {[
                { label: 'Total Sales', value: earnings?.grossSales ?? earnings?.netEarnings ?? 0, icon: 'fa-chart-line', color: '#0ea5e9' },
                { label: `Commission (${Math.round((earnings?.commissionRate ?? 0.1) * 100)}%)`, value: earnings?.commission || 0, icon: 'fa-percent', color: '#f59e0b' },
                { label: 'Net Earnings', value: earnings?.netEarnings ?? 0, icon: 'fa-coins', color: '#10b981' },
