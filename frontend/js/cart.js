...
// Get user-specific cart key based on session
function getCartKey() {
  // Check if user is logged in via NextAuth session
  const sessionData = document.querySelector('meta[name="user-id"]');
  if (sessionData) {
    return `hilgod_cart_${sessionData.getAttribute('content')}`;
  }
  return CART_KEY; // Fallback to generic cart for non-logged-in users
}

// Merge anonymous cart with user cart on login
function mergeCartsOnLogin() {
  const anonCart = JSON.parse(localStorage.getItem(CART_KEY)) || [];
  if (anonCart.length === 0) return; // Nothing to merge
  ...
  anonCart.forEach(anonItem => {
    const exists = mergedCart.find(item => item.key === anonItem.key);
    if (!exists) {
      mergedCart.push(anonItem);
    } else {
      // If item exists, update quantity
      exists.qty = Math.min(exists.qty + anonItem.qty, 99);
    }
  });
